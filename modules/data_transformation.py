...
def incremental_update(file_path, last_processed_id, id_column):
    ...
    if id_column in data.columns:
        ...
        print(f"ID column '{id_column}' not found in the data. Unable to perform incremental update.")
...
def main():
    ...
    last_processed_customer_id = incremental_update(
        'data_dir/Market 1 Customers.json', last_processed_customer_id, id_column='Customer_ID')

    # Incremental update for orders data
    last_processed_orders_id = incremental_update(
        'data_dir/Market 1 Orders.csv', last_processed_orders_id, id_column='Order_ID')

    # Incremental update for deliveries data
    last_processed_deliveries_id = incremental_update(
        'data_dir/Market 1 Deliveries.csv', last_processed_deliveries_id, id_column='Delivery_ID')

    # Load data
    customers_data = load_json(
        'data_dir/Market 1 Customers.json')
    orders_data = load_csv('data_dir/Market 1 Orders.csv')
    deliveries_data = load_csv(
        'data_dir/Market 1 Deliveries.csv')
